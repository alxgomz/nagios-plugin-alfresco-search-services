...
class alfSolrRes(nagiosplugin.Resource):
	"""Get the Solr statistics values out of the status and summary
	reports generated by Alfresco's Solr server. Only valid for Solr 4
	and above (shipped in Alfresco 5.x and above)"""
	
	def __init__(self, args):
		self.solrClientCertFile = '/tmp/solr_ssl_client.crt'
		self.solrClientKeyFile = '/tmp/solr_ssl_client.key'
...
def main():
	...
	parser.add_argument('--admin', default='/solr/admin/cores', help='Cores admin URL of the Solr service')
	...
	parser.add_argument('--monitor', choices=['fts','index','handlers','caches'], \
		help='Specify what Solr part (group of items) we want to monitor', required = True)
	...
	parser.add_argument('--item', help='Name of item to monitor (see above for details)', required = True)
